...
const saveChanges = recordElement => {
  ...
  if (isInvalidInput(patientNameInput.value) === true || isInvalidInput(symptomsInput.value) == true) {
...
const onRecordSubmit = event => {
  ...
  //To check if any of the field is empty
  if (isInvalidInput(patientName.value) === true || isInvalidInput(symptoms.value) == true) {
    messageModal("Invalid input: Please enter a non-empty value.");
  } else {
    const newRecord = {
